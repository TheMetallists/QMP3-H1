...
void processing_threadV(int isInverted) {
    ...
    setbuf(stdout, NULL);
     ...
     gpx.option.inv = (i8_t)isInverted;
    ...
    if (!option_softin) {
        ...
        if (1) {
            ...
            //dsp.fp = NULL; // file?
            dsp.sr = 44100; // sample_rate
            dsp.bps = 16; // bits_sample  bits/sample
            dsp.nch = 1; // channels
            dsp.ch = 0; // selected channel
