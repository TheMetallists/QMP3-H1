...
static int f32read_sample(dsp_t *dsp, float *s) {
    ...
    for (i = 0; i < dsp->nch; i++) {
        //printf("[WORDUP] %lld\n",(long long)word);
        if (sound_read(word, dsp->bps/8, 1) < dsp->bps/8) return EOF;
...
static int f32read_csample(dsp_t *dsp, float complex *z) {
    ...
    if (dsp->bps == 32) { //float32
        float f[2];
        if (sound_read( f, dsp->bps/8, 2) != 2) return EOF;
        x = f[0];
        y = f[1];
    }
        ...
        short b[2];
        printf("BZZ: %ll\n",(long long)b);
        if (sound_read( b, dsp->bps/8, 2) != 2) return EOF;
        x = b[0]/32768.0;
        y = b[1]/32768.0;
    }
    else {  // dsp->bps == 8   //uint8
        ui8_t u[2];
        if (sound_read( u, dsp->bps/8, 2) != 2) return EOF;
        x = (u[0]-128)/128.0;
        y = (u[1]-128)/128.0;
    }
...
static int f32read_cblock(dsp_t *dsp) {
    ...
    len = sound_read( s, dsp->bps/8, 2*dsp->decM) / 2;
